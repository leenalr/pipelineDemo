        ...
        stage('Scan') {
             steps {
                script {
                    ...
                    sh 'curl -sfL https://raw.githubusercontent.com/aquasecurity/trivy/master/contrib/install.sh | sh sudo -s -- -b /usr/local/bin'
                    sh 'trivy image flaskapp:$BUILD_NUMBER'
                }
              }
        }
